...
sub insertion {
    ...
    if($option == 1) {
        ...
        my $format = interface("ask_format", 0, 0);
        ...
        my $seq = Bio::Seq->new(-seq => $sequence, -id => $id_sequence, -alphabet => $alphabet, -is_circular => $is_circular);
        ...
        else {$format = interface("ask_format", 0, 0);}
        ...
        my $answer = interface("ask_database");   # Escolher a Base de Dados
        if ($answer ==1) { interface("genbank_importation_begin"); genbank_importation(); }    # Escolheu :     GENBANK    
        elsif ($answer ==2) { interface ("swissprot_importation_begin");}
        elsif ($answer ==3) { interface ("NCBI_importation_begin");}
...
sub insert_sequence{
    my ($id_sequence, $seq) = @_;
    $dbm_seq{$id_sequence} = $seq;
...
sub create_file{
    ...
    my $seq = $dbm_seq{$row->{id_sequence}};#Bio::Seq->new(-seq => $dbm_seq{$row->{id_sequence}}, -display_id => $row->{gene_name}, -accession_number => $row->{accession_number}, -desc => $row->{description}, -alphabet => $row->{alphabet});
    my $seqio;
...
sub update_info{
    ...
    my $sql = "UPDATE sequences SET gene_name='".$seq->display_id."', accession_number='".$seq->accession_number."', description='".$seq->desc."', alphabet='".$seq->alphabet."', length='".
                     length($seq->seq)."' WHERE id_sequence='".$id_sequence."'";
    $dbh->do($sql);
    $dbm_seq{$id_sequence} = $seq;
}
...
sub interface {
    ...
    if($type eq "welcome"){
        ...
        $option = <>;
        if($option == 1 or $option == 2 or $option == 3 or $option == 4) {return $option;}
        ...
        if($clear) {system $^O eq 'MSWin32' ? 'cls' : 'clear';}
        if($invalid) {print "INVALID OPTION! Please choose a valid one! ";}
        print "Insert the alphabet [dna/rna/protein]: ";
        $answer = <>;
        chomp $answer;
        if($answer eq "dna" or $answer eq "rna" or $answer eq "protein") {return $answer;}
        ...
        if($clear) {system $^O eq 'MSWin32' ? 'cls' : 'clear';}
        if($invalid) {print "FORMAT NOT SUPORTED! Please choose a suported one!";}
        print "Insert the format [fasta/genbank/swiss]: ";
        $answer = <>;
        chomp $answer;
        if($answer eq "fasta" or $answer eq "genbank" or $answer eq "swiss") {return $answer;}
        else{interface("ask_format", 0, 1);}
    }
